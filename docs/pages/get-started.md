...
* **Compiler**: After the model has been developed and its structure fixed, the compiler is used to convert its computational flow into binary instructions for specific accelerators, which is usually done on a host isolated from the chip.
