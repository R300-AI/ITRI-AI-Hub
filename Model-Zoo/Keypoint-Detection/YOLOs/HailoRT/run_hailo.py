...
with InferVStreams(network_group, input_vstreams_params, output_vstreams_params) as infer_pipeline:
    with network_group.activate(network_group_params):
        ...
        #-----[inference]-----
        outputs = infer_pipeline.infer({input_vstream_info[0].name: im})
        latents = {name_space[i]: np.transpose(outputs[i], (0, 3, 1, 2)).astype(np.float32) for i in outputs}
        results = session.run(output_details, latents)
